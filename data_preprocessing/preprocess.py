...
def clean_silvaco_output(output_text):
    ...
    for line in lines:
        ...
        if 'Gateway' in line and ('Spice Netlist Generator' in line or 'Netlist Generator' in line):
            should_skip = True
...
def show_before_after_examples(input_file, num_examples=3):
    ...
    for i in range(min(num_examples, len(data))):
        ...
        if original != cleaned:
            ...
            print(f"Reduction:       {len(original) - len(cleaned)} chars ({((len(original)-len(cleaned))/len(original)*100):.1f}%)")
